...
def run_simulation(q0, v0, simu_params):
    ...
    if(simu_params['use_exp_int']):
        simu = consim.build_exponential_simulator(dt, ndt, robot.model, robot.data,
                                    conf.K, conf.B , conf.mu, conf.anchor_slipping_method, False)
    else:
...
#    simu.init(q0, v0, p0=conf.p0)
    ...
    p0  = np.zeros((nk, N_SIMULATION+1))
    dp0 = np.zeros((nk, N_SIMULATION+1))
    dp0_qp = np.zeros((nk, N_SIMULATION+1))
    dp = np.zeros((nk, N_SIMULATION+1))
    dp_fd = np.zeros((nk, N_SIMULATION+1))
    dJv = np.zeros((nk, N_SIMULATION+1))
    dJv_fd = np.zeros((nk, N_SIMULATION+1))
