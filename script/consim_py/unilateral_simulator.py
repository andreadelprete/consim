...
class RobotSimulator:
    ...
    def check_contacts(self):
        ...
        for (i,c) in enumerate(self.contacts):
            ...
            if c.active:
                self.n_active_ += 1 
        if not (f.shape[0]==3*self.n_active_):
            self.f = zero(3*self.n_active_)
            self.Jc = np.zeros((3*self.n_active_, 3*self.n_active_))
...
#            se3.cholesky.computeMinv(self.model, self.data)
    ...
    def compute_exponential_LDS(self, u, update_expm):
        ...
        if(update_expm):
            ...
            self.a[self.nk:] = JMinv.dot((self.S.T.dot(u)-h)) + self.dJv
            self.A[self.nk:, :self.nk] = -self.Upsilon.dot(self.K)
            self.A[self.nk:, self.nk:] = -self.Upsilon.dot(self.B)
    ...
    def step(self, u, dt=None, use_exponential_integrator=True, dt_force_pred=None, ndt_force_pred=None,
        ...
        if(not use_exponential_integrator):            
            ...
            v_mean = self.v + 0.5*dt*dv_bar + JMinv.T.dot(D_int2_x/dt)
    ...
    def checkFrictionCone(self,D_int_x ,dt): 
        ...
        for i,c in enumerate(self.contacts):
            ...
            normal_force_value = c.normal.T.dot(f_average[3*i_active:3*i_active+3])[0,0]
