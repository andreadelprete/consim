...
GROUND_TRUTH_IMP_EUL_SIMU_PARAMS = {
    'name': 'ground-truth-imp %d'%(2**i_ground_truth),
...
tau_0 = 0*np.array([2., 0., -0.19, 0., 0., 0.])
tau_A = 0*np.array([2., 0.,  0.,   0., 0., 0.])
...
for simu_params in SIMU_PARAMS:
    ...
    if(simu_params['simulator']=='implicit-euler'):
        data[name] = run_simulation(conf, dt, N, robot, controller, q0, v0, simu_params)
    else:
        data[name] = run_simulation(conf, dt, N, robot, controller, q0, v0, simu_params)
                            
print("\nStart simulation ground truth")
data['ground-truth-implicit-euler'] = run_simulation(conf, dt, N, robot, controller, q0, v0, GROUND_TRUTH_IMP_EUL_SIMU_PARAMS)
